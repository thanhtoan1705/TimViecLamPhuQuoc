<x-filament-panels::page>
    <div class="content-wrapper">
        ...
        <div class="candidate-grid">
            @foreach($applyCandidates as $candidate)
                @php
                    if (isset($candidate->candidate->user->avatar_url))
                        $img = asset('storage/' . $candidate->candidate->user->avatar_url);
                    else
                        $img = asset('default/user.png');
                @endphp
                <div class="candidate-card"><i class="fa-solid fa-user"></i>
                    <div class="card-content">
                        <img src="{{ $img }}" alt="{{ $candidate->candidate->user->name }}" class="candidate-avatar">
                    ...
                    <div class="candidate-details">
                        ...
                        <div class="details-cv">
                            <span class="cv">
                                <a class="details-cv-cv" href="
                                    @if ($candidate->file)
                                        {{ asset('storage/' . $candidate->file) }}
                                    @elseif ($candidate->candidate->resume_id)
                                        {{ asset('storage/' . $candidate->candidate->resume->file) }}
                                    @else
                                        N/A
                                    @endif
                        ...
                        <button
                            wire:click="unApplyCandidate({{ $candidate->job_post_id }}, {{ $candidate->candidate_id }})"
                            class="button-un-save"
    ...
    <style>
        ...
        .details-cv .details-cv-cv:hover {
