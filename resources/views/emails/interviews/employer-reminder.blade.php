...
<head>
    ...
    <style>
        p { font-size: 16px !important; line-height: 25px; }
    </style>
</head>
...
<div style="border: 1px solid #ddd; border-radius: 5px; padding: 20px; margin: 20px auto; width: 100%; max-width: 700px;">
    <div style="margin-top: 15px;">
        <strong style="display: inline-block; text-align: center; font-size: 18px; width: 100%;">
            Nhắc nhở: Bạn có lịch phỏng vấn sắp diễn ra
        </strong>

        <div style="margin-top: 20px;">
            <p>Kính gửi {{ $interview->employer->user->name }},</p>
            ...
            <ul>
                ...
                @if($interview->interview_type === 'online')
                    ...
                    <li>Link phỏng vấn: <a href="{{ $interview->zoom_start_url }}" style="color: #007bff;">{{ $interview->zoom_start_url }}</a></li>
                @else
            ...
            <p><strong>Danh sách ứng viên:</strong></p>
            <ul>
                @foreach($interview->candidates as $candidate)
                    <li>{{ $candidate->user->name }} - {{ $candidate->user->email }}</li>
                @endforeach
            </ul>
        </div>
        ...
        @if($interview->interview_type === 'online')
            <p style="text-align: center;">
                <a href="{{ $interview->zoom_start_url }}"
